      ...
      <header className="bg-slate-900/80 backdrop-blur-xl border-b border-slate-800 sticky top-0 z-50">
        <div className="px-6 py-3 grid grid-cols-[auto_1fr_auto] items-center gap-4">
          ...
          <div className="flex items-center justify-center gap-6">
            <div className="flex items-center gap-3">
              <span
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold tracking-wide transition-all ${state.isRunning ? "bg-emerald-500/20 text-emerald-400 shadow-sm shadow-emerald-500/20" : "bg-slate-800 text-slate-500"}`}
              >
                {state.isRunning && (
          ...
          <div className="p-5 border-b border-slate-800/60">
            <div className="flex items-center justify-between mb-4">
              ...
              {state.activeScenario && (
                <span className="px-2.5 py-1 bg-blue-500/20 text-blue-400 text-xs rounded-full font-medium">
                  vs Baseline
                </span>
              )}
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {kpiData.map((kpi) => {
                ...
                const iconColor = kpi.color === 'blue'
                  ? 'text-blue-400'
                  : kpi.color === 'red'
                    ? 'text-red-400'
                    : kpi.color === 'amber'
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-baseline justify-between">
                      ...
                      {showDelta && (
                        <div className={`flex items-center gap-1 text-xs font-medium px-2 py-0.5 rounded ${isBetter ? 'text-emerald-400 bg-emerald-500/10' : 'text-red-400 bg-red-500/10'}`}>
          ...
          <div className="flex-1 flex flex-col overflow-hidden">
            <div className="px-5 pt-4 pb-2 flex items-center justify-between">
              ...
              <span className="text-xs text-slate-500">
                {
                  SCENARIO_CATEGORIES.find((c) => c.type === activeScenarioType)
                    ?.label
                }
              </span>
            </div>

            {/* Scenario Tabs - Clean horizontal list */}
            <div className="px-5 pb-6">
              <div className="flex flex-wrap gap-2">
                {SCENARIO_CATEGORIES.map((cat) => (
                  ...
                  >
                    <span className="text-base">{scenarioIcons[cat.type]}</span>
                    <span>{cat.label}</span>
                  </button>
                ))}
              </div>
            ...
            <div className="flex-1 px-6 py-6 overflow-y-auto">
              {activeScenarioType === "baseline" &&
                (() => {
                  const operational = state.stations.filter(
                  ...
                  const activeChargers = state.stations.reduce(
                  ...
                  const inventoryPct =
                    totalCap > 0 ? (totalInventory / totalCap) * 100 : 0;
                  const totalSwaps = state.stations.reduce(
                    (sum, s) => sum + s.totalSwaps,
                    0,
                  );
                  const avgWait =
                    totalStations > 0
                      ? state.stations.reduce(
                          (sum, s) => sum + s.avgWaitTime,
                          0,
                        ) / totalStations
                      : 0;
                  const lowStockStations = state.stations.filter(
                    (s) => s.currentInventory / s.inventoryCap < 0.25,
                  ).length;
                        ...
                        <div className="flex-1">
                          <div className="text-sm font-medium text-white">
                            {operational === totalStations
                              ...
                              : `${operational}/${totalStations} Operational`}
                          </div>
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div className="bg-slate-800/50 border border-slate-700/50 rounded-lg p-4">
                          ...
                          <div className="text-xl font-bold text-emerald-400 tabular-nums">
                            {operational}<span className="text-sm text-slate-600 ml-0.5">/{totalStations}</span>
                          </div>
                        </div>
                        <div className="bg-slate-800/50 border border-slate-700/50 rounded-lg p-4">
                          <div className="text-xs text-slate-500 mb-2 flex items-center gap-2">
                          ...
                          <div className="text-xl font-bold text-blue-400 tabular-nums">
                            {activeChargers}<span className="text-sm text-slate-600 ml-0.5">/{totalChargers}</span>
                          </div>
                        </div>
                        <div className="bg-slate-800/50 border border-slate-700/50 rounded-lg p-4">
                          <div className="text-xs text-slate-500 mb-2 flex items-center gap-2">
                          ...
                          <div className={`text-xl font-bold tabular-nums ${avgUtil > 85 ? 'text-red-400' : avgUtil > 65 ? 'text-amber-400' : 'text-emerald-400'}`}>
                        ...
                        <div className="bg-slate-800/50 border border-slate-700/50 rounded-lg p-4">
                          ...
                          <div className={`text-xl font-bold tabular-nums ${avgWait > 8 ? 'text-red-400' : avgWait > 4 ? 'text-amber-400' : 'text-emerald-400'}`}>
                            {avgWait.toFixed(1)}<span className="text-sm text-slate-600"> min</span>
                          </div>
                        </div>
                      </div>

                      {/* Inventory bar - simplified */}
                      <div className="bg-slate-800/50 border border-slate-700/50 rounded-lg p-4">
                        ...
                        <div className="h-2 bg-slate-700 rounded-full overflow-hidden">
                          <div
                            className={`h-full rounded-full transition-all ${inventoryPct < 25 ? 'bg-red-500' : inventoryPct < 50 ? 'bg-amber-500' : 'bg-emerald-500'}`}
                            style={{ width: `${inventoryPct}%` }}
                          />
                        </div>
                        <div className="flex justify-between mt-2">
                          <span className="text-xs text-slate-500">
                            {totalInventory.toLocaleString()} / {totalCap.toLocaleString()} batteries
                          </span>
                          {lowStockStations > 0 && (
              ...
              {activeScenarioType === "failures" && (
                <div className="space-y-4">
                  <p className="text-sm text-slate-400">
                    Simulate emergencies to test network resilience and driver
                    rerouting
                  </p>
                  <div className="grid grid-cols-2 gap-2.5">
                  ...
                  <div className="p-3 bg-red-500/10 border border-red-500/20 rounded-xl">
                    <p className="text-xs text-red-300">
                      ...
                      {state.stations.find((s) => s.id === targetStationId)
                        ?.name || "Station"}{" "}
                      will be affected. Drivers will auto-reroute.
                    </p>
                  </div>
                </div>
              )}
              ...
              {activeScenarioType === "capacity" && (
                <div className="space-y-5">
                  <p className="text-sm text-slate-400">
                    Modify charger capacity at stations to test impact on
                    throughput
                  </p>
                  <div>
                    <label className="text-sm text-slate-300 font-medium mb-2 block">
                ...
                <div className="space-y-5">
                  <div className="p-3 bg-slate-800/50 rounded-xl">
                    ...
                    <p className="text-xs text-slate-500">
                      Increase prices during peak hours to balance demand.
              ...
              {activeScenarioType === "inventory" && (
                <div className="space-y-5">
                  <div className="p-3 bg-slate-800/50 rounded-xl">
                    ...
                    <p className="text-xs text-slate-500">
                      Set safety stock levels and rebalancing rules to optimize
              ...
              {activeScenarioType === "network" && (
                <div className="space-y-4">
                  <p className="text-sm text-slate-400">
                    Take stations offline to test network resilience. Drivers
                    will auto-reroute to remaining stations.
                  </p>
                  <div>
                    <label className="text-sm text-slate-300 font-medium mb-2.5 block">
                      Select Stations to Disable
                    </label>
                          ...
                          return (
                            ...
                            >
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <div className="text-xs text-slate-500">
                                  {s.chargers} chargers · {s.location}
                                </div>
                              </div>
                            </button>
                          );
                  ...
                  {networkDisabledStations.size > 0 && (
                    <div className="p-3 bg-red-500/10 border border-red-500/20 rounded-xl">
                      <p className="text-xs text-red-300">
                        <AlertTriangle size={12} className="inline mr-1" />
                        {networkDisabledStations.size} station(s) will go
                        offline.{" "}
                        {state.stations.filter((s) => s.status !== "offline")
                          .length - networkDisabledStations.size}{" "}
                        remaining to serve all drivers.
                      </p>
                    </div>
                  )}
                </div>
              ...
              {activeScenarioType === "growth" && (
                <div className="space-y-5">
                  <div className="p-3 bg-slate-800/50 rounded-xl">
                    ...
                    <p className="text-xs text-slate-500">
                      Model how increasing EV adoption affects network
              ...
              {activeScenarioType === "combined" && (
                <div className="space-y-4">
                  <p className="text-sm text-slate-400">
                    Stack multiple interventions to test compound effects on the
                    network.
                  </p>

                  {/* Demand toggle */}
                  <div
                    className={`rounded-xl border transition-all ${combinedDemand ? "bg-blue-500/10 border-blue-500/30" : "bg-slate-800/50 border-slate-700/50"}`}
                  >
                    <button
                    ...
                    {combinedDemand && (
                      <div className="px-3.5 pb-3.5">
                        <input
                          ...
                          onChange={(e) =>
                            setCombinedDemandValue(Number(e.target.value) / 100)
                          }
                          className="w-full"
                        />
                  ...
                  {/* Capacity toggle */}
                  <div
                    className={`rounded-xl border transition-all ${combinedCapacity ? "bg-emerald-500/10 border-emerald-500/30" : "bg-slate-800/50 border-slate-700/50"}`}
                  >
                    <button
                    ...
                    >
                      ...
                      <div>
                        ...
                        <div className="text-xs text-slate-500">
                          Extra chargers at all stations
                        </div>
                      </div>
                    </button>
                    {combinedCapacity && (
                      <div className="px-3.5 pb-3.5">
                        ...
                        <div className="text-xs text-emerald-400 mt-1">
                          +{combinedCapacityValue} chargers per station
                        </div>
                      </div>
                    )}
                  </div>
                  ...
                  {/* Pricing toggle */}
                  <div
                    className={`rounded-xl border transition-all ${combinedPricing ? "bg-amber-500/10 border-amber-500/30" : "bg-slate-800/50 border-slate-700/50"}`}
                  >
                    <button
                    ...
                    {combinedPricing && (
                      <div className="px-3.5 pb-3.5">
                        ...
                        <div className="text-xs text-amber-400 mt-1">
                          {combinedPricingValue}x peak pricing
                        </div>
                      </div>
                    )}
                  </div>
            ...
            <div className="flex gap-4 text-xs">
              {[
                ...
                <div key={item.label} className="flex items-center gap-1.5">
                  <div
                    className={`${item.small ? "w-2 h-2" : "w-2.5 h-2.5"} rounded-full ${item.color}`}
                  />
